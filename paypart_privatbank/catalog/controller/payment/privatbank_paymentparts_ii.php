...
//        $data_deal['partsCount'] = $this->config->get('privatbank_paymentparts_ii_merchantType');
        $partsCount = $this->config->get('privatbank_paymentparts_ii_merchantType');
        $partsCountArr = array();
        for($i=$partsCount;$i>=2;$i--){
            $partsCountArr[] = $i;           
        }
        $data['partsCounts'] = $partsCountArr;
//		$data['testmode'] = $this->config->get('privatbank_paymentparts_ii_test');
        
		if (!$this->config->get('privatbank_paymentparts_ii_test')) {
//            $data['action'] = 'https://www.paypal.com/cgi-bin/webscr';
//			$data['action'] = 'https://payparts2.privatbank.ua/ipp/v2/payment/create';            
//            $data['action'] = 'http://localhost.loc/Opencart_2_1_0_2/mockAPI/paymentCreate.php';
...
//        print_r($this->session->data['order_id']);
		...
		if ($order_info) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/privatbank_paymentparts_ii.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/payment/privatbank_paymentparts_ii.tpl', $data);
			} else {
				return $this->load->view('default/template/payment/privatbank_paymentparts_ii.tpl', $data);
			}
		}
	}
...
//         print_r($productsString);exit;
    ...
    public function sendDataDeal(){
        ...
        if ($order_info) {
            ...
            $data_deal['responseUrl'] = $this->url->link('payment/privatbank_paymentparts_ii/callback', '', 'SSL');
...
//            $data_deal['redirectUrl'] = $this->url->link('payment/privatbank_paymentparts_ii/callback', '', 'SSL');
