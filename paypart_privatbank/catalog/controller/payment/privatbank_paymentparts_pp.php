...
//            $data['action'] = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
        ...
        if ($order_info) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/privatbank_paymentparts_pp.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/payment/privatbank_paymentparts_pp.tpl', $data);
            } else {
                return $this->load->view('default/template/payment/privatbank_paymentparts_pp.tpl', $data);
            }
        }
    }
...
//         print_r($productsString);exit;
    ...
    public function sendDataDeal(){
        ...
        if ($order_info) {
            ...
            $data_deal['responseUrl'] = $this->url->link('payment/privatbank_paymentparts_pp/callback', '', 'SSL');
